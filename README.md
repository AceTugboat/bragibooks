...
To run Bragibooks as a container, you need to pass some paramaters in the run command:
  ...
  | `-e HOSTED_DOMAIN=bragibooks.example.com` | **Production only** - Your domain name. Automatically configures CORS and CSRF settings for both `https://yourdomain.com` and `https://www.yourdomain.com`. Leave unset for local development. |
  | `-e CSRF_TRUSTED_ORIGINS=https://bragibooks.mydomain.com` | **Advanced** - Manually set trusted domains (comma-separated). Only use if you need different CORS/CSRF origins than HOSTED_DOMAIN provides. |
